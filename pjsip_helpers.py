...
def build_account_config(bind_ip: str, transport_id: int) -> pj.AccountConfig:
    ...
    # The From user stays the SIP account, ALWAYS. Asterisk identifies an inbound
    # endpoint by this field, so putting a caller ID here instead matched no
    # endpoint, dropped the call onto PJSIP/anonymous, and it never reached the
    # AI context at all. Caller ID travels in P-Asserted-Identity instead.
